...
<section id="login" v-bind:class="isShake">
 <form method="post" action="{{route('process_sign_up')}}">
    {{csrf_field()}}
  <h2>Sign Up</h2>
  ...
  <input type="text" name="Ho_ten" v-model="login.login" placeholder="Fullname" />
  ...
  <input type="text" name="Ten_co_quan" v-model="login.login" placeholder="work place" />
